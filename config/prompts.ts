...
直接生成图片，不要输出文字描述。`,
  ...
  generateDescription: `你是一位资深的信息可视化设计师。请根据讲稿内容，设计这一页**信息图风格**的PPT画面。

**【设计理念】**
参考 NotebookLM、McKinsey 报告的风格：每页是一个完整的信息单元，图文结合，逻辑清晰。

**【每页必须包含】**
1. **标题**（8-15字）：概括核心观点
2. **结构化要点**（2-4条，每条8-20字）：
   - 这是内容的主体，要言之有物
   - 用序号或卡片形式组织
3. **可视化图示**：
   - 流程/步骤 → 流程图、时间线
...
直接输出设计方案，不要解释。`,
  ...
  generateDescriptionWithTemplate: `你是一位资深的信息可视化设计师。根据讲稿内容设计**信息图风格**的画面。

**参考模板图片的风格**：配色、背景、装饰元素。
...
直接输出设计方案，不要解释。`,
  ...
  // 内容页图片生成（有模板）
  generateImage: `生成一张**信息图风格**的PPT页面图片。
...
1. **文字排版要有设计感**：
   - 标题用醒目的字体、可以有色块背景
   - 要点文字用卡片/色块/圆角框/标签等承载，不要裸露直接打字
   - 每个要点可以配小图标
   - 文字占页面 40-60%

2. **图示是辅助**：
   - 配合文字内容的可视化图示
   - 可以是流程图、图标卡片、关系图、场景插画等
   - 图示占页面 40-60%

3. **布局要求**：
   - 参考模板的整体风格和布局
   - 常见布局：左文右图、上标题下内容、卡片式等
   - 信息层次清晰，一眼能看懂

4. **视觉风格**：
   - 专业、简洁、现代
   - 配色协调，参考模板
   - 文字要有装饰：背景色块、卡片边框、图标点缀等
...
16:9比例，直接生成图片。`,
  ...
  // 内容页图片生成（无模板）
  generateImageNoTemplate: `生成一张**信息图风格**的PPT页面图片。
